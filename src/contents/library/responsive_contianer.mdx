---
title: 'Creating a Responsive Tailwind Container in React'
tags: 'react'
publishedAt: '2021-11-13'
description: 'Setting up Absolute Imports with jsconfig'
---

> Creating a Responsive Tailwind Container in React
...
const ResponsiveContainer = ({ children }) => {
   return (
      <div className="w-full max-w-sm m-4">{children}</div>
   );
};
